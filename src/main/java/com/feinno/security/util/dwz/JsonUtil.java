...
public class JsonUtil {
	 ...
	 * @param backMsg
	 * @param element
	 *            节点名称：orderid,resultCode,sessionkey
	 * @return
	 */
	public static String parseJsonBackMsg(String json, String element) {
		if (StringUtil.isEmpty(json)) {
			return null;
		}
		if (logger.isDebugEnabled()) {
			logger.debug("[back msg]=====> \n" + json);
		}
		String value = null;
		try {
			BackMsg msg = JsonUtil.fromJson(json, BackMsg.class);
			if (msg != null) {
				Field[] fields = Class.forName("com.feinno.pay.entity.BackMsg")
						.getDeclaredFields();
				Field field = null;
				for (int i = 0; i < fields.length; i++) {
					field = fields[i];
					field.setAccessible(true); // 设置些属性是可以访问的
					if (element.equalsIgnoreCase(field.getName())) {
						value = String.valueOf(field.get(msg));
						break;
					}
				}
			}
